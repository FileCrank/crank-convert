...
lazy_static! {
    pub static ref FILE_TYPES: Vec<&'static FileType<'static>> = vec![TXT.deref(), HTML.deref()];
    pub static ref FILE_TYPE_NAMES: HashMap<&'static str, &'static FileType<'static>> = {
        let mut file_type_name_map: HashMap<&'static str, &'static FileType<'static>> =
            HashMap::new();
    ...
    pub static ref FILE_TYPE_EXTENSIONS: HashMap<&'static str, &'static FileType<'static>> = {
        let mut file_type_extension_map: HashMap<&'static str, &'static FileType<'static>> =
            HashMap::new();
        for file_type_ref in FILE_TYPES.deref() {
            let file_type = file_type_ref.deref();
