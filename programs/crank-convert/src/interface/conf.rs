...
use std::fs::File;
use std::ops::Deref;
use crate::errors::{Result, ConversionError::InvalidConfigurationError};
...
fn parse_opts_holder(opts: &Opts) -> Result<DataHolder> {
    match (opts.data, opts.file) {
        (Some(mut d), None) => Ok(DataHolder::Raw(d.to_owned())),
        (None, Some(f)) => read_file_to_holder(f),
        (Some(_), Some(_)) => Err(InvalidConfigurationError("Only data or file should be provided, not both")),
...
pub fn parse_opt_file_type<'a>(oft: OptFileType<'a>) -> Result<&'a FileType<'static>> {
    match oft {
        OptFileType::Name(s) => {
...
pub fn parse_validate_opts<'a>(opts: Opts<'a>) -> Result<ConversionConf<'a>> {

    match parse_opts_holder(&opts) {
        Ok(mut holder) => Ok(ConversionConf {
